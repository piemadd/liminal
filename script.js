...
let preBackgroundObject = document.getElementById('preImage');
let button = document.getElementById('button');
let audio = document.getElementById("audio");
let imageURL = images[Math.floor(Math.random() * images.length)];

let preloadedImages = [];
const preload = ((imagesArr) => {
    for (let i = 0; i < imagesArr.length; i++) {
        preloadedImages[i] = new Image();
        preloadedImages[i].src = imagesArr[i];
    }
})

backgroundObject.style['background-color'] = '#000';

const switchImage = (() => {
    console.log(imageURL);
    backgroundObject.style['background-image'] = "url(../pics/" + imageURL + ")";
    
    imageURL = images[Math.floor(Math.random() * images.length)];
    preload(["../pics/" + imageURL]);
})

function play() {
    audio.play()

    backgroundObject.style['background-color'] = ''
